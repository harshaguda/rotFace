...
base_model=InceptionV3(weights='imagenet',include_top=False, input_shape = (299, 299, 3), pooling = 'avg') #imports the mobilenet model and discards the last 1000 neuron layer.
...
train_generator=train_datagen.flow_from_directory('/home/rignitc/srinath/rotFace-masteroriginal/half_face_train', # this is where you specify the path to the main data folder
                                                 target_size=(299,299),
                                                 color_mode='rgb',
                                                 batch_size=64,
...
val_generator=train_datagen.flow_from_directory('/home/rignitc/srinath/rotFace-masteroriginal/half_face_validate', # this is where you specify the path to the main data folder
                                                 target_size=(299,299),
                                                 color_mode='rgb',
                                                 batch_size=64,
                                                 class_mode='categorical',
...
filepath="inceptionv3_halffaces_weights.best.hdf5"
...
history = model.fit_generator(generator=train_generator,
        ...
        epochs=200, validation_data = val_generator, validation_steps = val_steps, callbacks=callbacks_list)
...
model.save('inceptionv3_halffaces.h5')
print(history.history.keys())
plt.subplot(211)
plt.plot(history.history['acc'])
plt.plot(history.history['val_acc'])
plt.title('Accuracy for Inceptionv3 model')
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
plt.title('Loss for Inceptionv3 model')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train', 'validate'], loc='upper left')
plt.tight_layout()
plt.savefig('inceptionv3_halffaces.png')


with open('inceptionv3.txt', 'wb') as file_pi:
	pickle.dump(history.history, file_pi)

# model.load_weights("/home/rignitc/srinath/rotFace-masteroriginal/inceptionv3_halffaces_weights.best.hdf5")
# predict = []
# for i in range (1,23):
...
#     img = image.load_img("/home/rignitc/srinath/rotFace-masteroriginal/half_face_test/%d.jpg"%i, target_size = (299,299)) #---->  KERAS WAY
#     #print("%d.jpg"%i)
