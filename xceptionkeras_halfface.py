...
base_model=Xception(weights='imagenet',include_top=False, input_shape = (299, 299, 3), pooling = 'avg') #imports the mobilenet model and discards the last 1000 neuron layer.
...
filepath="xception_halffaces_weights.best.hdf5"
...
model.save('xception_halffaces.h5')
print(history.history.keys())
plt.subplot(211)
plt.plot(history.history['acc'])
plt.plot(history.history['val_acc'])
plt.title('Accuracy for Xception model')
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
plt.title('Loss for Xception model')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train', 'validate'], loc='upper left')
plt.tight_layout()
plt.savefig('xception_halffaces.png')





with open('xception.txt', 'wb') as file_pi:
    pickle.dump(history.history, file_pi)




model.load_weights("/home/rignitc/srinath/rotFace-masteroriginal/xception_halffaces_weights.best.hdf5")
predict = []
for i in range (1,23):
    ...
    img = image.load_img("/home/rignitc/srinath/rotFace-masteroriginal/half_face_test/%d.jpg"%i, target_size = (224,224)) #---->  KERAS WAY
    #print("%d.jpg"%i)
